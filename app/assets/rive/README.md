# Mascottes Rive — fennec et cigogne

`fennec.riv` et `cigogne.riv` conservent les calques WebP des personnages acceptés, avec articulation du corps et de la tête. Chaque fichier inclut ses images. Il ne contient ni police ni ressource distante.

- Artboards : `Fennec` et `Cigogne` ; machine : `Companion`.
...
Le contrôleur `p4q_rive.js` anime le fennec et la cigogne, conserve les calques HTML en secours, libère les instances retirées et suspend le rendu invisible. Le réglage système de réduction des animations désactive Rive. Le chameau et le palmier conservent leur animation existante. La cigogne utilise une respiration plus discrète, des déplacements verticaux réduits et des inclinaisons du cou plus douces que le fennec.
...
Validation : `node tests/test_rive.js` (Playwright) teste les deux personnages : vrai rendu WASM, le mouvement, les réactions, le nettoyage, la suspension, le secours et la réduction des animations ; `node tests/test_design_pages.js` couvre les écrans FR/AR.
